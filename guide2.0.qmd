...
WildTrax is the leading platform to help you manage, store and process your environmental sensor data while sharing and discovering the biological data from them that you can share with others. With WildTrax you can:
...
- **Create a [Project](#sec-aru-projects)**: Establish a project to process your data.  
- **Add Users**: Invite team members to your organization and project for collaborative data management.  
- **Upload Media**: Upload and organize media files for processing.  
- **Create or Upload Tasks/Surveys**: Define tasks or upload surveys to guide data processing.  
...
WildTrax operates under a role-based access control system, meaning a user in the system represents an individual, not an organization or a group of people. This restricts access to authorized users and is a policy-neutral mechanism designed to create roles and privileges. These users can then collaborate to manage data or share data to answer broader scientific questions. This means that you may not have access to certain features, tools or data depending on your **permissions level**. Permission to project data for example can be limited by the [location settings](#sec-location-settings), and project publication status.

## Membership and privacy settings

WildTrax is designed with flexible roles and permissions to ensure secure and tailored access to its features. Your account can have different levels of access, depending on your role and privileges. Here's an overview of the key permissions levels:

- [Organization membership](#sec-manage-organization-members) all you to see or govern access to all projects and data under an Organization. Permissions at the Organization level include:
  - Administrator
  - Read-Only Member
...
The interplay of these different permissions level ultimately affect what data you see and how you can interface with data on WildTrax. Organizations are parent to projects, therefore any Organization Administrator automatically get Project Administrator privileges. Conversely, if a user is added to a project as a read-only member, they will not have access to the Organization which is parent to the project.
...
![](assets/data-discover-landing-page.png)
...
Click on My Data in the top ribbon, followed by My Organizations. This will take you to the organization dashboard. Click the ![](assets/create-organization){height=40px} button, from here the Organization Settings form will appear.
...
- [**Amazon Web Services (AWS)**](https://aws.amazon.com/): Retained as an option for users requiring cloud-based storage with global accessibility and high-performance capabilities with international storage base options. 
...
### Manage organization members {#sec-manage-organization-members}
...
data <- data.frame(
  ...
  Description = c(
    ...
    "Privacy setting: 'Visible,' 'Hidden – Location,' or 'Hidden – Location+Data'",
    "Exact ('True Location') or offset ('Buffered Location')",
  ...
  Example = c(
    ...
    "Visible", "True Location", "0", "Mountain Range Area"
  )
)
...
::: {.callout-tip collapse="true" style="background-color: #f4f4f4; padding: 20px;"}
Did you know locations are created in the organization when media are uploaded even via a project? If a location name already exists in the organization, WildTrax will append the media to it, and if it is a new location WildTrax will even prompt you during the upload process if you want to add spatial coordinates to these new locations.

![](assets/aru-upload-coordinates.png)
:::

### Location settings and privacy options {#sec-location-settings}

You can control each location’s settings by clicking on the ![](assets/pencil.png){height=20px} beside the location name. Location settings control who and what users can see across the system. WildTrax provides multiple ways for organization administrators to control how WildTrax members can see locations and data collected at them. These location privacy options keep data protected when needed. 
...
- You can enter coordinates for the true location in the form and then instruct WildTrax to provide a buffer (indicated in the buffer radius (m) field)
...
- Next, within the Organization view, click on the Locations tab and then on  next to any location.
- At the bottom of the location settings you should see a link under "Report Link". Copy this link. Open the link in your browser or share it with the desired recipient.
...
Location photos are photos taken as a 

### Uploading location photos

Each location photo will also have the following properties that can be assigned to each photo. 
...
- **Access**: the accessibility of the visit image; this is determined by organization administrators (see [Default image privacy settings]())
- Visit Image Comments: any useful information about the visit image


### Syncing location photo properties

## Visits

A **visit** is when an observer has gone to a location to collect environmental sensor or biological data. When equipment is placed at a location during a visit, WildTrax calls this a deployment. Conversely, if you are visiting a location to pick up a sensor, or the data it collected, this is its retrieval. At each of these times, you can add sensor (i.e., equipment) metadata to support the visit. Conversely, when media are uploaded into WildTrax, there is the capacity to generate the visit automatically. 
...
Within the DOIs tab, users can create DOIs via [DataCite]() that allows users to create DOIs links for their projects. There are certain restrictions for creating DOIs:
