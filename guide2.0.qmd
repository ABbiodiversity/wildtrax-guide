...
- **Projects** are groups of specific media to address research questions or implement study designs. Projects can be one of three sensors: [ARUs](#sec-aru-projects) for acoustic data, [cameras](#sec-camera-projects) or [point counts](#sec-point-count-projects).

![](assets/projects.png){.lightbox}

- **Tasks** are specific assignments given to users to process media, such as audio recordings or image sets, into biological data. Within each task, species **tags** are applied to extract meaningful information, such as species presence or abundance. Equivalent to tasks in the [point count](#sec-point-count-projects) sensor are **surveys** with species **observations** further contributing to the biological data available.

::: {style="display: flex; justify-content: center; gap: 40px;"}
<img src="assets/aru-processing.png" alt="ARU Processing" style="width: 45%; height: auto;"/><img src="assets/camera-processing.png" alt="Camera Processing" style="width: 45%; height: auto;"/>
:::
...
## Publishing, sharing and downloading data {#sec-publishing-and-sharing}
...
Reports in WildTrax can be accessed at the project level. They provide high and low level detail about the data in the project and provide everything you need for downstream analysis and decision-making. Right-click, select multiple or click the pencil icon and select ![](assets/download-report.png){height="30px"}
...
::: {.callout-tip collapse="true" style="background-color: #f4f4f4; padding: 20px;"}
Did you know locations are created in the organization when media are upload either via [the organization](#sec-upload-recordings) or [a project](#sec-upload-recordings-tasks)? If a location name already exists in the organization, WildTrax will append the media to it, and if it is a new location WildTrax will prompt you during the upload process if you want to add spatial coordinates to these new locations.
...
A **visit** occurs when an observer goes to a location to collect environmental sensor or biological data. If equipment is placed at the location during a visit, this is referred to in WildTrax as a [deployment](#sec-deployments). Visits allow for adding sensor metadata to standardize field data collection, making it easier to share with other organizations or WildTrax users.

## Why are Visits Important?

-   **Quality Control:** Visit metadata ensures accuracy for environmental sensor media and associated metadata.
-   **Data Integration:** Visits link field activities to equipment, media, and biological data.
-   **Contextual Information:** Visits document general landscape details where sensors are placed or data is collected.

## Do You Need to Collect Visit Data?

Visit metadata is not mandatory; however, you cannot upload visit images or equipment metadata without recording a visit.

## What Visit Data Does WildTrax Support?

Visits typically include two main activities:\
1. Using an environmental sensor.\
2. Conducting a point count.

WildTrax supports the addition of general landscape information and associated deployment details.

## Creating Visits

1.  Navigate to the **Visits** tab in your organization.
2.  Click **Create Visit** and select a location from the list.
3.  Complete the visit summary form that displays all visits and equipment for the location.
4.  Enter the visit date (required).\
5.  Click **Save** to enable the **Add Sensor** button and access additional metadata options.

## Syncing Visits

You can batch upload and download visits for easier management:

1.  Click **Manage** and choose **Download Visits** to get a CSV template or existing visit data.\
2.  Edit the CSV with your visit information.\
3.  Click **Upload Visits**, select your file, and preview the changes.\
4.  Apply changes if no issues are detected.

### CSV Fields for Syncing Visits

-   **Location:** Name of the location.\
-   **Visit Date:** Date of the visit (format: YYYY-MM-DD).\
-   **Crew:** Names of crew members.\
-   **Bait/Lure Type:** Type of attractant used (if applicable).\
-   **Access Method:** Method of reaching the location (e.g., “On Foot,” “Helicopter”).\
-   **Walktest Distance (m):** Distance at which walktests are conducted.\
-   **Walktest Height (m):** Height at which walktests are conducted.\
-   **Facing Human Feature:** Flags if a camera or ARU is near a human feature.\
-   **Visit Trigger Mode(s):** Camera activation settings (e.g., “Trigger,” “Time-lapse”).\
-   **Motion Image Interval (s):** Time between images during motion activation.\
-   **Quiet Period (s):** Delay between trigger events.\
-   **Visit Comments:** Additional details about the visit.

> **Tip:**\
> Batch uploads support **add** and **update** operations only; deletions are not allowed. For example, an empty CSV upload will not delete existing data.

## Deleting Visits

You can delete visits using the ![](assets/delete-menu.png){height="30px"} option, accessible via the ![](assets/dropdown-arrow.png){height="20px"}. Deletion will not be allowed until all dependencies, such as media, are removed.

![](assets/delete-visit-form.png){.lightbox}
...
For any DOI in the menu, right-click or select through the ![](assets/dropdown-arrow.png){height="28px"} the ![](assets/delete-doi.png){height="30px"} option. You can also delete multiple DOIs by selecting multiple DOIs and going to ![](assets/manage.png){height="30px"}. You'll be prompted to go through the process of confirming the deletion prior to it taking place.

![](assets/delete-dois-form.png){.lightbox}

::: {.callout-question collapse="true" style="background-color: #f9f9f9; border-left: 5px solid #dc3545; padding: 15px; margin: 15px 0; color: #343a40;"}
If you've read up to the end of this section, you're pretty much an expert at Organizations! Now, discover more about the data processing and sharing side with your first projects.
:::

# Projects {#sec-projects}

**Projects** are purposeful groupings of an organization‘s media and metadata to answer specific questions or implement a study design. Projects can be one of three sensors: [ARUs](#sec-), cameras or point counts. For each of these sensor types, a collection of tasks (ARU), image sets (camera) or surveys (point counts) are processed or uploaded in order retrieve species data.
